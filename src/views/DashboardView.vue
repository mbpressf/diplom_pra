...
const capitalEquationText = computed(() => {
  return `${money(financeStore.vault.balance || 0)} + ${money(financeStore.vault.available_to_spend || 0)} = ${money(financeStore.vault.net_balance || 0)}`
})
      ...
      <div class="relative z-[1] grid gap-5 lg:grid-cols-[1.3fr_0.7fr] lg:items-end">
        <div class="space-y-3">
          ...
          <div class="rounded-3xl border border-white/15 bg-slate-950/8 p-3.5 dark:bg-white/5 sm:p-4">
            <p class="text-sm font-medium text-slate-700 dark:text-slate-200">{{ t('dashboardHowMoneyWorks') }}</p>
        ...
        <div class="hidden gap-3 lg:grid">
          <div class="rounded-3xl border border-white/15 bg-white/68 p-4 dark:bg-slate-950/38">
            ...
            <p class="mt-2 text-xl font-semibold text-slate-900 dark:text-slate-50 sm:text-2xl">{{ money(financeStore.vault.net_balance || 0) }}</p>
          </div>
          <div class="rounded-3xl border border-emerald-400/25 bg-emerald-500/10 p-4">
            ...
            <p class="mt-2 text-xl font-semibold text-accent sm:text-2xl">{{ money(financeStore.vault.balance || 0) }}</p>
          </div>
          <div class="rounded-3xl border border-brand-500/20 bg-brand-500/10 p-4">
            ...
            <p class="mt-2 text-xl font-semibold text-brand-700 dark:text-brand-100 sm:text-2xl">{{ money(financeStore.vault.available_to_spend || 0) }}</p>
          </div>
        </div>
      </div>
    </article>
